...
export function ImportSkillModal({ open, management, onClose, onInstalled }: ImportSkillModalProps) {
  ...
  return <Modal open={open} label="导入技能" className="import-skill-dialog" onClose={close}>
    ...
    {!staged ? <>
      ...
      <dl><dt>来源文件</dt><dd>{staged.fileName}</dd><dt>文件</dt><dd>{staged.inspection.files.length} 个，{sizeOf(staged.inspection.totalBytes)}</dd><dt>安装范围</dt><dd><Select value={scope} onChange={event => setScope(event.currentTarget.value as SkillInstallScope)}><option value="shared-agents">所有 NexusOne 任务（共享）</option><option value="profile">当前 Harness Profile</option></Select></dd></dl>
