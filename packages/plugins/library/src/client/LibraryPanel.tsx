...
export function LibraryPanel({ management, previewRegistry, toggleNavigation, startConversation }: Props) {
  ...
  return <section className="wd-library"><style>{libraryCss}</style>
    ...
    {menu ? <div className="wd-library-popover" style={{ left: menu.x, top: menu.y }} onClick={event => event.stopPropagation()}>{menu.create ? <><button onClick={() => { setDialog({ kind: 'create-markdown', parentId: menu.parentId, value: '未命名文档.md' }); setMenu(undefined); }}>新建文档（.md）</button><button onClick={() => { setDialog({ kind: 'create-text', parentId: menu.parentId, value: '未命名文本.txt' }); setMenu(undefined); }}>新建文本（.txt）</button><button onClick={() => { setDialog({ kind: 'create-folder', parentId: menu.parentId, value: '新建文件夹' }); setMenu(undefined); }}>新建文件夹</button><hr/><button onClick={() => { uploadParent.current = menu.parentId; upload.current?.click(); setMenu(undefined); }}>上传和导入</button></> : menu.entry ? <>{menu.entry.kind === 'folder' ? <><button onClick={event => showMenu(event, { create: true, parentId: menu.entry!.id })}>在此新建或导入</button><hr/></> : null}<button onClick={() => void addToTask(menu.entry!)}>添加到新对话</button><hr/><button onClick={() => { beginMove(menu.entry!); setMenu(undefined); }}>移动到…</button><button onClick={() => { setDialog({ kind: 'rename', entry: menu.entry!, value: menu.entry!.name }); setMenu(undefined); }}>重命名</button>{menu.entry.asset ? <button onClick={() => { menu.entry!.asset?.status === 'disabled' ? void setStatus(menu.entry!) : setDialog({ kind: 'status', entry: menu.entry!, value: '' }); setMenu(undefined); }}>{menu.entry.asset.status === 'disabled' ? '重新启用' : '停用'}</button> : null}<button className="danger" onClick={() => { setDialog({ kind: 'remove', entry: menu.entry!, value: '' }); setMenu(undefined); }}>删除</button></> : null}</div> : null}
